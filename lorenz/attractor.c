...
//lorenz point array and number of points
int numPts = 50000;
float arrPts[50000][3];

//window viewing parameters
...
static void display(void)
{
  ...
  glLoadIdentity();
    ...
    //print parameters at the bottom of the gl window
    glWindowPos2i(5,5);
    Print("s = %lf, b = %lf, r = %lf",s,b,r);

    /* Draw the graph of the attractor */
    glBegin(GL_LINE_STRIP); //GL_LINE_STRIP will connect the dots sequentially
      int i = 0;

      while(i < iteration && i < numPts)
      {
        glColor3d(i/70000.0, 1 - i/80000.0, 1 - i/1000000.0);  //colors the image as time moves
...
static void reshape(int width, int height)
{
  ...
  glMatrixMode(GL_PROJECTION); // Allows us to manipulate the projectoin matrix in OpenGL
  glLoadIdentity();
  glFrustum(-2.0, 2.0, -h*2, h*2, 1.0, 200.0); //identifies the cutting planes and clips image
...
static void key (unsigned char key, int x, int y)
{
  switch (key)
  {
    case 27:
      exit(0);
      break;
    // adjust s parameter up and down
    case 's':
      s += 1.0;
      break;
    case 'x':
    ...
    // adjust b parameter up and down
    case 'd':
      b += 0.3333;
      break;
    case 'c':
...
int main(int argc, char* argv[]) {
  ...
  glutCreateWindow("Rock Hw2 - Lorenz Attractor");
